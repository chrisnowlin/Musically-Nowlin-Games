import React, { useEffect, useState } from 'react';
import { getGoogleAuthUrl, isGoogleAuthEnabled } from './api';

const TeacherLoginPage: React.FC = () => {
  const [googleEnabled, setGoogleEnabled] = useState<boolean | null>(null);

  useEffect(() => {
    isGoogleAuthEnabled().then(setGoogleEnabled);
  }, []);

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-purple-950 flex items-center justify-center">
      <div className="bg-slate-800 rounded-xl p-8 shadow-xl max-w-md w-full text-center">
        <h1 className="text-2xl font-bold text-white mb-2">Melody Dungeon</h1>
        <h2 className="text-lg text-purple-300 mb-6">Teacher Dashboard</h2>
        <p className="text-slate-400 text-sm mb-8">
          Create custom question pools for your students. Sign in with your Google account to get started.
        </p>
        {googleEnabled === null ? (
          <p className="text-slate-500 text-sm">Checking login availability...</p>
        ) : googleEnabled ? (
          <a
            href={getGoogleAuthUrl()}
            className="inline-flex items-center gap-3 bg-white text-slate-800 px-6 py-3 rounded-lg font-semibold hover:bg-slate-100 transition-colors"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
            </svg>
            Sign in with Google
          </a>
        ) : (
          <div className="space-y-3">
            <p className="text-amber-400 text-sm font-medium">Google sign-in is not configured yet.</p>
            <p className="text-slate-500 text-xs">
              Set <code className="bg-slate-700 px-1 rounded">GOOGLE_CLIENT_ID</code> and{' '}
              <code className="bg-slate-700 px-1 rounded">GOOGLE_CLIENT_SECRET</code> in your{' '}
              <code className="bg-slate-700 px-1 rounded">.env.local</code> file to enable teacher login.
            </p>
          </div>
        )}
      </div>
    </div>
  );
};

export default TeacherLoginPage;
