...
export default function StaffInvadersGame() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('staffInvadersHighScores');
    if (saved) {
      try {
    ...
    // Load SFX preference
    const sfxPref = localStorage.getItem('staffInvadersSFX');
    if (sfxPref === 'false') {
    ...
    const showCorrectPref = localStorage.getItem('staffInvadersShowCorrectAnswer');
  ...
  useEffect(() => {
    localStorage.setItem('staffInvadersHighScores', JSON.stringify(state.highScores));
  }, [state.highScores]);

  // Save SFX preference
  useEffect(() => {
    localStorage.setItem('staffInvadersSFX', String(state.sfxEnabled));
    audioService.setVolume(state.sfxEnabled ? 0.3 : 0);
  }, [state.sfxEnabled]);

  // Save show correct answer preference
  useEffect(() => {
    localStorage.setItem('staffInvadersShowCorrectAnswer', String(state.showCorrectAnswer));
