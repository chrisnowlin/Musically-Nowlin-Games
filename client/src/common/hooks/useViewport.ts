...
export const useResponsiveLayout = () => {
  ...
  const getSafePadding = () => {
    if (width < BREAKPOINTS.xs) return 2; // Very small devices
    if (width < BREAKPOINTS.sm) return 4; // Small mobile
    if (width < BREAKPOINTS.md) return 6; // Mobile
    if (width < BREAKPOINTS.lg) return 8; // Tablet
    return 10; // Desktop
  };
  ...
  const getGridGap = () => {
    if (width < BREAKPOINTS.sm) return 4; // 0.25rem
    if (width < BREAKPOINTS.md) return 6; // 0.375rem
    if (width < BREAKPOINTS.lg) return 8; // 0.5rem
    if (width < BREAKPOINTS.xl) return 10; // 0.625rem
    return 12; // 0.75rem
  };
  ...
  const getAvailableHeight = () => {
    const headerHeight = device.isMobile ? 40 : 45;
    const footerHeight = device.isMobile ? 25 : 30;
