# Enemy Sprite Art (Skeleton & Goblin) Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace placeholder ghost-copy PNGs for the skeleton and goblin enemy types with unique pixel art sprites using a Python/Pillow generation script.

**Architecture:** Write `scripts/generate-enemy-sprites.py` that defines each sprite as a 64×64 logical pixel grid (each logical pixel = 32×32 actual pixels → 2048×2048 output), draws it with Pillow using nearest-neighbor scaling, and saves the result to `client/public/images/`. No changes to game code required — the `ENEMY_SPRITE` map in `DungeonGrid.tsx` already maps to the correct paths.

**Tech Stack:** Python 3, Pillow (`from PIL import Image, ImageDraw`)

---

### Task 1: Scaffold the generation script with the skeleton sprite

**Files:**
- Create: `scripts/generate-enemy-sprites.py`
- Output: `client/public/images/melody-dungeon-skeleton.png`

**Background — how the grid works:**
The script uses a `SCALE = 32` constant. Each "pixel" in the 64×64 design is drawn as a
32×32 filled rectangle on the 2048×2048 canvas. A helper `px(x, y, color)` draws one
logical pixel. Transparent cells are skipped (leave canvas alpha = 0).

**Step 1: Create the script with skeleton pixel art**
...
Save this file as `scripts/generate-enemy-sprites.py`.

**Step 2: Run the script**

```bash
cd /Users/cnowlin/Developer/Musically-Nowlin-Games
python3 scripts/generate-enemy-sprites.py
```

Expected output:
```
Saved: .../client/public/images/melody-dungeon-skeleton.png
Saved: .../client/public/images/melody-dungeon-goblin.png
```

**Step 3: Visually verify both sprites**

Open the generated images in Preview or any image viewer:
```bash
open client/public/images/melody-dungeon-skeleton.png
open client/public/images/melody-dungeon-goblin.png
```

Verify:
- Skeleton: white bone skull, red glowing eyes, ribcage structure, music note in right hand
- Goblin: green round head, large ears, red squinting eyes, jagged teeth, purple collar, music note floating nearby
- Both have transparent backgrounds (checkered in Preview)
- Neither looks like the ghost sprite

**Step 4: Adjust pixel art if needed**

The `draw_skeleton()` and `draw_goblin()` functions define every pixel via `p(x, y, color)` calls
on a 64×64 logical grid. To adjust:
- Move a feature: change the x/y coordinates
- Change a color: swap the palette constant (e.g., `BW` → `BM`)
- Add details: add more `p()` calls in the appropriate function
Re-run step 2 after each adjustment.

**Step 5: Commit**

```bash
git add scripts/generate-enemy-sprites.py \
        client/public/images/melody-dungeon-skeleton.png \
        client/public/images/melody-dungeon-goblin.png
git commit -m "feat: add unique pixel art sprites for skeleton and goblin enemies"
```

---

### Task 2: Verify sprites render correctly in-game

**Files:**
- Read (no edits): `client/src/components/melody-dungeon/DungeonGrid.tsx:48-53`

**Step 1: Confirm ENEMY_SPRITE map is already correct**

Open `client/src/components/melody-dungeon/DungeonGrid.tsx` and verify lines 48-53:
```typescript
...
```
No changes needed — the map already points to the correct paths.

**Step 2: Start the dev server and navigate to a dungeon floor with skeleton/goblin enemies**
...
Navigate to the game and enter a dungeon. Skeleton and goblin enemy tiles should now render
with their new unique sprites instead of the purple ghost.

Enemy subtype assignment lives in `client/src/lib/gameLogic/dungeonGenerator.ts` — search for
`enemySubtype` to see which floors spawn which enemy types if you need to find the right floor
quickly.

**Step 3: No further code changes needed**

The `DungeonGrid.tsx` component renders sprites with a floating animation
(`animate-sprite-float`) applied to all enemy tiles automatically. The new sprites will inherit
this animation.
