...
test.describe('Beat & Pulse Trainer - Interactive Play-Test', () => {
  test('Manual interactive gameplay session with DevTools monitoring', async ({ page }) => {
    console.log('\n🎮 INTERACTIVE PLAY-TEST: Beat & Pulse Trainer');
    ...
    // Navigate to game
    console.log('1️⃣ Loading Beat & Pulse Trainer...');
    await page.goto('http://localhost:5175/games/beat-and-pulse-trainer');
    await page.waitForLoadState('networkidle');
    console.log('✅ Page loaded\n');

    // Take initial screenshot
    await page.screenshot({ path: `.playwright-mcp/beat-pulse-trainer-initial.png`, fullPage: true });
    console.log('📸 Screenshot: beat-pulse-trainer-initial.png\n');
    ...
    if (hasStartButton) {
      ...
      await page.screenshot({ path: `.playwright-mcp/beat-pulse-trainer-started.png`, fullPage: true });
      console.log('📸 Screenshot: beat-pulse-trainer-started.png\n');
    } else {
    ...
    const gameplayElements = {
      ...
      beat: await page.locator('text=/[Bb]eat|[Pp]ulse|[Rr]hythm/').isVisible().catch(() => false),
      buttons: await page.locator('button').count(),
    };

    console.log(`   Score display: ${gameplayElements.score ? '✅' : '⚠️'}`);
    console.log(`   Timer: ${gameplayElements.timer ? '✅' : '⚠️'}`);
    console.log(`   Beat/Pulse indicator: ${gameplayElements.beat ? '✅' : '⚠️'}`);
    ...
    await page.screenshot({ path: `.playwright-mcp/beat-pulse-trainer-final.png`, fullPage: true });
    console.log('📸 Screenshot: beat-pulse-trainer-final.png\n');
