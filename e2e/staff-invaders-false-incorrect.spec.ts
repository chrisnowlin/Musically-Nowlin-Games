...
test.describe('Staff Invaders intermittent false incorrect', () => {
  test('presses correct keys immediately after spawn to detect ref/race issues', async ({ page }) => {
    test.setTimeout(3 * 60 * 1000);

    resetDebugLog();
    await page.goto('/games/staff-invaders');

    // Setup screen loaded
    await expect(page.getByText('Staff Invaders')).toBeVisible();
  ...
  test('does not produce false incorrects when "Show Correct Answer" is enabled', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      localStorage.setItem('staffInvadersShowCorrectAnswer', 'true');
    });
